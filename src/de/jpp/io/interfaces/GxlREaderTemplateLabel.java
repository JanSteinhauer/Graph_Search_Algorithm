...
public abstract class GxlREaderTemplateLabel<N, A, G extends Graph<N, A>, F> implements GraphReader<N, A, G , F>{ //todo später wieder abstrakt machen

    Map<String, N> nodeMap = new HashMap<>(); //hier stand vorher XYNOde
    List<Element> nodeOrEdge = new ArrayList<>();
    List<String> letzeTags = new ArrayList<>();
    Map<String, String> label = new HashMap<>();

    public  GxlREaderTemplateLabel(){ //todo was soll diese Methode sagen
  ...
  /*  public static void main(String[] args) throws ParseException {
    ...
    public G read(F input) throws ParseException { //todo hier wird ein String übergeben
        ...
        for (int i = 0; i < children.size(); i++) {
            ...
            if(element.getName().equals("edge")){
                ...
                N zwischenspeichernode = readNode(element);//mit String
    ...
    private void addEdge(Graph graph, Element element, Map<String, N> map) throws ParseException {//todo Zwischenfrage, wäre es nicht hier auch schon schlau die nodes zu den Graphen hinzuzufügen
        N start,dest; //todo hier stand vorher XYNOde
        try {
            start = map.get(element.getAttributeValue("from"));
       }catch (Exception e){
           throw new IllegalArgumentException("es gibt probleme mit from");
       }
        try {
             dest = map.get(element.getAttributeValue("to"));
        }catch (Exception e){
            throw new IllegalArgumentException("es gibt probleme mit to");
        }
        /*System.out.println(element.getName());*/
        List<Element> unterpunkte = element.getChildren();
        Optional<Double> op = Optional.empty();
        for (int i = 0; i < unterpunkte.size(); i++) {
            /*System.out.println(unterpunkte.get(i).getAttributeValue("name"));*///todo so wie ich mir das Denke istOption mit der String Map
            String value = unterpunkte.get(i).getValue();
            value = value.replace(" ","");
            label.put(unterpunkte.get(i).getAttributeValue("name"),value);

        }

        if(label.isEmpty()){
            graph.addEdge(start,dest,Optional.empty());
        }else {
            graph.addEdge(start,dest,Optional.of(label));
        }
    ...
    abstract public N readNode(Element element) throws ParseException;
