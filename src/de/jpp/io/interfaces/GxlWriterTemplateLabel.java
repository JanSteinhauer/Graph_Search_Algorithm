...
public abstract class GxlWriterTemplateLabel<N, A, G extends Graph<N, A>, F> implements GraphWriter<N, A, G , F> { // classe noch abstract machen
    StringBuilder stringBuilder = new StringBuilder();
    List<N> nodes = new ArrayList<>();
    HashMap<N, List<Edge<N,A>>> edges = new HashMap<>();
    HashMap<Node, Integer> nodeId = new HashMap<>();
    HashMap<Integer,String> nodeId2 = new HashMap<>();
...
//        gxlWriterTemplate.write(graph);
    ...
    public  F write(G graph) throws ParseException {
        ...
        for (int i = 0; i < nodes.size(); i++) {
            N node = (N) nodes.get(i); //hier steckt der Fehler // hier durch string erstetzen
            Element element = writeNode((String) node);
            nodeId2.put(i,(String) node);
        }
...
//        }
    ...
    abstract public Element writeNode(String node);
